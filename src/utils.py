...
def process_CLUECorpusSmall(tokenizer, BATCH_SIZE, save_all_text = False):
    ...
    if check_is_processed('pretrain_CLUECorpusSmall', data_path):
...
def process_baidu(data_path, tokenizer, BATCH_SIZE, save_all_text=False):
    ...
    if check_is_processed('pretrain_baidubaike_563w', data_path):
...
def process_wiki_zh_clean(tokenizer, wiki_data_path, save_all_text=False):
    data_path = GLOBAL_DATA_PATH
    if check_is_processed('pretrain_wiki_zh', data_path):
...
def process_medical(data_path, name, tokenizer, save_all_text=False):
    data_path = GLOBAL_DATA_PATH
    if check_is_processed(f'pretrain_medical_{name}.bin',data_path):
        print(f'pretrain_medical has been processed')
        return

    if not os.path.exists(data_path):
        print(
            f'{data_path} is not exist. please download from: https://huggingface.co/datasets/shibing624/medical/blob/main/pretrain/medical_book_zh.json')
        return

    if save_all_text:
        corpus_txts = open(f'./data/tokenizer_medical_{name}.txt', 'w', encoding='utf-8')
...
def process_medical_qa(tokenizer, save_all_text=False):
    data_path = GLOBAL_DATA_PATH
    if check_is_processed('pretrain_medical_qa.bin',data_path):
...
def process_valid_medical(tokenizer, save_all_text=False):
    data_path = GLOBAL_DATA_PATH
    if check_is_processed('valid_data.bin',data_path):
    ...
    if save_all_text:
        corpus_txts = open(f'./data/tokenizer_valid_medical.txt', 'w', encoding='utf-8')
...
def process_test_medical(tokenizer, save_all_text=False):
    data_path = GLOBAL_DATA_PATH
    if check_is_processed('test_data.bin',data_path):
...
def sft_process(save_all_text=False):
    data_path = GLOBAL_DATA_PATH
    if check_is_processed('sft_data.csv',data_path):
